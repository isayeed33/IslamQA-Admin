...
import { getFaq, updateFaq, deleteFaq } from "../../helpers/api/islamqa";
...
const FaqEdit = () => {
  ...
  const { register, handleSubmit, control, watch, reset, formState: { errors, isSubmitting } } = useForm<FaqFormData>({
  ...
  useEffect(() => {
    ...
    getFaq(Number(id))
      ...
      .catch(() => { Swal.fire("Error", "Failed to load FAQ.", "error"); navigate("/faqs"); })
  ...
  const onSubmit = async (data: FaqFormData) => {
    ...
    try {
      ...
      await Swal.fire({ title: "FAQ Updated!", icon: "success", timer: 1800, showConfirmButton: false });
      navigate("/faqs");
    } catch (err: any) {
  ...
  const handleDelete = async () => {
    const result = await Swal.fire({ title: "Delete FAQ?", icon: "warning", showCancelButton: true, confirmButtonColor: "#E63535", confirmButtonText: "Delete" });
  ...
  if (loading) return (
    <>
      <PageBreadcrumb title={`Edit FAQ #${id}`} name={`Edit FAQ #${id}`} breadCrumbItems={["IslamQA", "FAQs", "Edit"]} />
            ...
            <div className={activeLang !== "en" ? "hidden" : "space-y-5"}>
              <div className="card p-5">
                ...
                <div className="mb-4">
                  ...
                  <input {...register("question")} type="text" className={`form-input w-full ${errors.question ? "border-red-500" : ""}`} />
                ...
                <div>
                  ...
                  <Controller name="answer" control={control} render={({ field }) => (
                    <QuillEditor value={field.value || ""} onChange={field.onChange} error={!!errors.answer} />
                  )} />
            ...
            <div className={activeLang !== "ru" ? "hidden" : "space-y-5"}>
              <div className="card p-5">
                ...
                <div className="mb-4">
                  ...
                  <input {...register("question_ru")} type="text" className="form-input w-full" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1.5">
          ...
          <div className="space-y-5">
            ...
            <div className="card p-4 border-danger/30 bg-danger/5">
              ...
              <button type="button" onClick={handleDelete} className="btn w-full border-danger text-danger hover:bg-danger hover:text-white text-sm py-2">
                <i className="mgc_delete_2_line me-1"></i> Delete this FAQ
              </button>
            </div>
          </div>
