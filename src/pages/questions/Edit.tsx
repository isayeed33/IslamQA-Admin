...
import { getQuestion, updateQuestion, deleteQuestion, getCategories, ApiCategory } from "../../helpers/api/islamqa";
...
const QuestionsEdit = () => {
  ...
  const { register, handleSubmit, control, watch, reset, formState: { errors, isSubmitting } } = useForm<QAFormData>({
  ...
  useEffect(() => {
    if (!id) return;
    Promise.all([
      getQuestion(Number(id), "en"),
      getQuestion(Number(id), "ru").catch(() => null),
      getCategories("en"),
    ]).then(([en, ru, cats]) => {
      setCategories(cats);
      reset({
        title: en.title, questionText: en.questionText,
        answerText: en.answerText, summaryText: en.summaryText,
        scholarName: en.scholarName, categoryId: en.categoryId,
        status: en.status, isNew: en.isNew, isEssential: en.isEssential, isTranscript: en.isTranscript ?? false,
        ...
        title_ru:        ru?.title        ?? "",
        questionText_ru: ru?.questionText ?? "",
        answerText_ru:   ru?.answerText   ?? "",
        summaryText_ru:  ru?.summaryText  ?? "",
        scholarName_ru:  ru?.scholarName  ?? "",
      });
    }).catch(() => {
  ...
  const onSubmit = async (data: QAFormData) => {
    ...
    try {
      ...
      await Swal.fire({ title: "Q&A Updated!", text: "Changes saved successfully.", icon: "success", timer: 2000, showConfirmButton: false });
      navigate("/questions");
    } catch (err: any) {
  ...
  const handleDelete = async () => {
    const result = await Swal.fire({ title: "Delete Q&A?", text: "This will permanently delete the question and its answer.", icon: "warning", showCancelButton: true, confirmButtonColor: "#E63535", confirmButtonText: "Delete" });
  ...
  if (loading) {
    return (
      <>
        <PageBreadcrumb title={`Edit Q&A #${id}`} name={`Edit Q&A #${id}`} breadCrumbItems={["IslamQA", "Q&A", "Edit"]} />
        ...
        <div className="grid xl:grid-cols-3 gap-6">
            ...
            <div className={activeLang !== "en" ? "hidden" : "space-y-5"}>
              <div className="card p-5">
                ...
                <div className="mb-4">
                  ...
                  <input {...register("title")} type="text" className={`form-input w-full ${errors.title ? "border-red-500" : ""}`} />
                ...
                <div>
                  ...
                  <textarea {...register("questionText")} rows={4} className={`form-input w-full ${errors.questionText ? "border-red-500" : ""}`} />
              ...
              <div className="card p-5">
                ...
                <div className="mb-4">
                  ...
                  <Controller name="answerText" control={control} render={({ field }) => (
                    <QuillEditor value={field.value || ""} onChange={field.onChange} error={!!errors.answerText} />
                  )} />
                ...
                <div>
                  ...
                  <Controller name="summaryText" control={control} render={({ field }) => (
                    <QuillEditor value={field.value || ""} onChange={field.onChange} error={!!errors.summaryText} />
                  )} />
            ...
            <div className={activeLang !== "ru" ? "hidden" : "space-y-5"}>
              <div className="card p-5">
                ...
                <div>
                  ...
                  <textarea {...register("questionText_ru")} rows={4} className="form-input w-full" />
                </div>
              </div>
              ...
              <div className="card p-5">
                ...
                <div className="mb-4">
                  ...
                  <input {...register("scholarName_ru")} type="text" className="form-input w-full" />
                </div>
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-600 dark:text-gray-300 mb-1.5">
          ...
          <div className="space-y-5">
            ...
            <div className="card p-5">
              ...
              <label className={`flex items-start gap-3 p-3 rounded-lg cursor-pointer border transition-colors mb-3 ${isNew ? "border-info/40 bg-info/5" : "border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30"}`}>
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">Shows in the "New Answers" section on the homepage</p>
                </div>
              </label>
              <label className={`flex items-start gap-3 p-3 rounded-lg cursor-pointer border transition-colors mb-3 ${isEssential ? "border-warning/40 bg-warning/5" : "border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30"}`}>
              ...
              <label className={`flex items-start gap-3 p-3 rounded-lg cursor-pointer border transition-colors ${isTranscript ? "border-purple-400/40 bg-purple-50 dark:bg-purple-900/10" : "border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/30"}`}>
            ...
            <div className="card p-4 border-danger/30 bg-danger/5">
              ...
              <button type="button" onClick={handleDelete} className="btn w-full border-danger text-danger hover:bg-danger hover:text-white text-sm py-2">
                <i className="mgc_delete_2_line me-1"></i> Delete this Q&A
              </button>
            </div>
          </div>
